...
HARDWARE_DESCRIPTION = (
    "Cloud station simulator + Python Raspberry Pi 4B logical client; "
    "Wokwi circuit is a visual reference"
    if LOCAL_REVIEW_MODE
    else "Wokwi ESP32 sensor/actuator node + Python Raspberry Pi 4B logical client"
)
CONTROLLER_DESCRIPTION = (
    "Cloud station simulator + private Python Raspberry Pi client"
    if LOCAL_REVIEW_MODE
    else "Wokwi ESP32 + public Python Raspberry Pi client"
)
...
class PublicFederatedEngine:
    ...
    def infer_and_update(
        ...
        fs.STATE.station(
            station,
            phase=(
                "Cloud-local regulation"
                if live and LOCAL_REVIEW_MODE
                else "Live MQTT regulation"
                if live
                else "Safety standby — zero output"
            ),
            ...
            if all_live:
                for station in fs.STATIONS:
                    ...
                    self.infer_and_update(
                        ...
                        live=True,
                        source=(
                            "local_review_controlled_perturbation"
                            if LOCAL_REVIEW_MODE and acknowledged_perturbation
                            else "local_review_synchronized_three_station_stream"
                            if LOCAL_REVIEW_MODE
                            else "mqtt_controlled_perturbation"
